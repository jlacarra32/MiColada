    ...
    <nav className="fixed bottom-0 w-full max-w-md mx-auto bg-zinc-900/80 backdrop-blur-xl border-t border-white/5 shadow-[0_-8px_30px_rgb(0,0,0,0.4)] pb-safe z-50">
      <div className="flex justify-around items-center h-[64px]">
        {navItems.map((item) => {
          ...
          if (item.special) {
            return (
              <Link key={item.href} href={item.href} className="flex flex-col items-center justify-center -mt-6 group relative z-10 w-16">
                ...
                <span className="text-[10px] font-bold text-zinc-400 mt-1 transition-colors group-hover:text-cyan-400">{item.label}</span>
              </Link>
            );
          }
          ...
          return (
            <Link
              ...
              href={item.href}
              className={clsx(
                "flex flex-col items-center justify-center w-16 transition-all duration-300 ease-in-out group relative",
                isActive ? "text-cyan-400" : "text-zinc-500 hover:text-zinc-300"
              )}
            >
              <div
                className={clsx(
                  "p-2 rounded-full transition-all duration-300 ease-in-out group-hover:-translate-y-1 group-active:scale-95",
                  isActive && "bg-cyan-400/10"
                )}
              >
                <Icon size={24} strokeWidth={isActive ? 2.5 : 2} />
              </div>
              <span className={clsx("text-[10px] mt-0.5 font-medium transition-colors", isActive && "font-bold text-cyan-400")}>
                {item.label}
              </span>
            </Link>
          );
